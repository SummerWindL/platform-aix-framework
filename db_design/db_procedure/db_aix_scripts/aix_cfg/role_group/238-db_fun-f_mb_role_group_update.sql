CREATE OR REPLACE FUNCTION f_mb_role_group_update(
...
AS $function$
/*
 * 函数说明：删除角色组。该函数只操作用户定义角色组。sysflag = 10000为系统角色
 * 参数：按要求填写参数
 *  * 返回：
 *     retcode： 小于0，失败；==0成功
 *     retvalue：返回成功或失败的信息
...
begin
	...
	update t_mb_role_group set
			rolegrpname = in_rolegrpname,
			rolegrpicon = in_rolegrpicon,
