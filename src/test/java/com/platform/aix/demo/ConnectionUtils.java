...
import static com.platform.aix.demo.FtpUtil.FTPConnect;

/**
 * @author Advance
 * @date 2021年09月26日 19:13
 * @since V1.0.0
 */
@Slf4j
...
//            pst.execute();
            ...
            sql ="select vouch_subnum,\n" +
                    ...
                    "       b_ccy\n" +
                    "\n" +
                    "  from bok_vouch_detail\n" +
                    "\n";
            //建立ftp连接
            FTPConnect(FtpUtil.ip,FtpUtil.port,FtpUtil.username,FtpUtil.password);
            //启动线程定时发送noop请求防止ftp服务器断开客户端连接  曲线救国，TODO 最好的办法是服务器设置断链时间，或者永不断链
            ScheduledExecutorService scheduledThreadPool = Executors.newScheduledThreadPool(5);
            t = scheduledThreadPool.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    log.info("delay 1 seconds, and excute every 60 seconds 定时向ftp服务器发送请求 keep Alive");
                    try {
            ...
            FileUtil.uploadFileByFTP1(FtpUtil.getFtpClient(), "F://","/","data.dat");
            FtpUtil.FTPDisConnect();//断开连接了直接停止定时任务
    ...
    private static void getDataByTableName(ResultSet rs,String sql,Connection con) throws SQLException {
        ...
        for (int r = 0; r < runnum; r++){
            ...
            rowSql = "SELECT *\n" +
                    "  FROM (SELECT rownum rn, t.* FROM ( "+sql+ " ) t ORDER BY rownum ASC)\n" +
                    " WHERE rn > " + lastrow;
            ...
            while(rs.next()){
                ...
                sb.append(CRLF);//分段读取肯定不是最后一行
                l++;
                if (k % 100000 == 0) {
                    // 创建文件，并追加内容
                    createFileByAddTo("F://data.dat", sb.toString(),"utf-8");
                    sb = new StringBuffer("");
                }
                k++;
            }
     ...
     * @param fileContent
     */
    public static void createFileByAddTo(String filePathAndName, String fileContent,String encode) {
        try {
            ...
            try {
                ...
                osw.write(fileContent); //写入内容
                log.info("------新建文件或追加文件成功！------");
            } catch (IOException e) {
                e.printStackTrace();
            }finally {   //关闭流
                try {
